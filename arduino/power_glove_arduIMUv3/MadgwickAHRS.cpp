...
// JRB: Altered slightly (and uncomprehendingly) by JRB, see link above
// JRB: for original sources.
...
void MadgwickAHRSupdate(struct MadgwickState *state, float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz) {
	...
	float sampleFreq = state->sampleFreq;

	// Use IMU algorithm if magnetometer measurement invalid (avoids NaN in magnetometer normalisation)
	if((mx == 0.0f) && (my == 0.0f) && (mz == 0.0f)) {
	        MadgwickAHRSupdateIMU(state, gx, gy, gz, ax, ay, az);
		return;
	}
